...
def should_send_alert(watch_type: str, watch_id: int, current_price: float) -> bool:
    """Check if we should send an alert (dedup: no similar alert in last N hours)."""
...
async def evaluate_and_alert_flight(watch, price_result):
    """Check if a flight price triggers an alert."""
    ...
    if price_result.raw_details.get("airline"):
        message += f"\n✈️ {price_result.raw_details['airline']}"
...
async def evaluate_and_alert_hotel(watch, price_result):
    """Check if a hotel price triggers an alert."""
    if watch.threshold_price is None:
        return
...
async def send_scraper_broken_alert(scraper_name: str, consecutive_failures: int, last_error: str):
    message = (
        f"*Scraper Alert* ⚠️\n"
        f"Scraper `{scraper_name}` has failed {consecutive_failures} times in a row.\n"
