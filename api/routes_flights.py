...
def list_flights():
    ...
    try:
        watches = session.query(FlightWatch).order_by(FlightWatch.created_at.desc()).all()
...
@flights_bp.route("/<int:watch_id>", methods=["GET"])
def get_flight(watch_id):
    session = SessionLocal()
    try:
        ...
        # Get latest prices per source
        latest_prices = (
            session.query(PriceRecord)
            .filter_by(watch_type="flight", watch_id=watch_id)
...
@flights_bp.route("/<int:watch_id>", methods=["PUT"])
def update_flight(watch_id):
    data = request.json
    session = SessionLocal()
    try:
        watch = session.query(FlightWatch).get(watch_id)
...
@flights_bp.route("/<int:watch_id>", methods=["DELETE"])
def delete_flight(watch_id):
    session = SessionLocal()
    try:
        watch = session.query(FlightWatch).get(watch_id)
        if not watch:
...
@flights_bp.route("/<int:watch_id>/prices", methods=["GET"])
def get_flight_prices(watch_id):
    session = SessionLocal()
    try:
